# `cog-2026-paper/`

LaTeX source of the CoG 2026 short paper that distils the thesis
contributions:

> **Combining Spatial and Entity-Based Reasoning for Competitive
> MicroRTS via U-Net and Transformers**

Status: under review for IEEE CoG 2026. The compiled PDF is shipped at
[`paper.pdf`](paper.pdf).

## Layout

| Path | What it is |
|---|---|
| [`paper.tex`](paper.tex) | The whole paper (single-file IEEEtran layout). |
| [`IEEEtran.cls`](IEEEtran.cls) | IEEE conference class. Vendored so build is reproducible across hosts. |
| [`references.bib`](references.bib) | Biblatex bibliography. |
| [`figures/`](figures/) | Figures used by the paper (subset of the dissertation's `figs-pdf/`, plus tighter layouts where needed). |

## How to compile

```bash
cd cog-2026-paper/
latexmk -pdf -interaction=nonstopmode paper.tex
```

## Anonymisation switch

`paper.tex` exposes a `\newif\ifanonymous` toggle in the preamble. Flip
`\anonymousfalse` to `\anonymoustrue` for the anonymised submission
version (authors blacked out, hyperref title set to "Anonymous"). The
build flow is unchanged.

## Cross-references

- The thesis itself is at [`../dissertation/`](../dissertation/).
- Every empirical claim in the paper is backed by an artefact under
  [`../data/`](../data/) (tournament CSVs, ablation eval grids,
  generalisation-probe table).
- The figures generated by [`../dissertation/figs/figs-python/`](../dissertation/figs/figs-python/)
  are the upstream source for the paper's figures.

## Notes

- Single-file `.tex` is intentional: the paper is short and a flat layout
  keeps merge conflicts trivial during peer-review iterations.
- Do not edit `references.bib` to use shortcuts (`\cite{paper}` over
  full keys): the bib file is shared with the dissertation and clean
  keys keep both builds aligned.
