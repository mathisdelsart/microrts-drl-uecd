  ...
  \begin{frame}[plain,noframenumbering]
    ...
    \begin{columns}[T]
      ...
      \begin{column}{0.52\textwidth}
        \footnotesize
        \planentry{1}{Context \& Objective}{1--5}%
        \planentry{2}{Background}{6--10}%
        \planentry{3}{State of the Art}{11--13}%
        \planentry{4}{Contributions}{14--15}%
        \planentry{5}{Evaluation Framework}{16--19}%
        \planentry{6}{Environment Stack}{20--23}%
        \planentry{7}{The UECD Architecture}{24--29}%
        \planentry{8}{Training Recipe}{30--32}%
        \planentry{9}{Results \& Interpretation}{33--49}%
        \planentry{10}{Discussion \& Conclusion}{50--52}%
      \end{column}
    \end{columns}
    \vfill
...
\begin{frame}[plain,noframenumbering]
  \begin{columns}[c]
    \begin{column}{0.62\textwidth}
      \raggedright\includegraphics[height=1.7cm,keepaspectratio]{logo_uclouvain_bg.png}
    \end{column}
    \begin{column}{0.38\textwidth}
      \raggedleft\includegraphics[height=1.7cm,keepaspectratio]{logo_epl_bg.png}
    \end{column}
  \end{columns}
...
\begin{frame}{Real-time strategy (RTS) games}
  \begin{columns}[c]
    ...
    \begin{column}{0.45\textwidth}
      \centering
      \begin{tikzpicture}
        \node[inner sep=0] (sc) {\includegraphics[width=0.82\linewidth]{starcraft2.pdf}};
      ...
      \begin{tikzpicture}
        \node[inner sep=0] (wc) {\includegraphics[width=0.82\linewidth]{warcraft3.png}};
        \node[anchor=north east, fill=white, rounded corners=1.5pt, inner sep=2.5pt,
...
% --- The PPO algorithm: introduce PPO + why it's the choice (first PPO slide) ---
\begin{frame}{The PPO Algorithm}
  \vfill
  \centering
  {\Large\textbf{\alert{P}}roximal \textbf{\alert{P}}olicy \textbf{\alert{O}}ptimization}\par
  \vfill
  {\small
  \begin{tabular}{@{}c@{\hspace{0.4cm}}l@{}}
    \schip{\faCheckCircle} & \textbf{The \alert{standard} on-policy algorithm} for MicroRTS DRL agents.\\[1.5em]
    \schip{\faCogs} & \textbf{A \alert{stable} training loop}, robust and well understood.\\[1.5em]
    \schip{\faVial} & \textbf{A clean \alert{testbed}}: every architecture and mechanism ablated in isolation.\\
  \end{tabular}}
  \vfill
\end{frame}
...
\iffalse % equations slide hidden from the deck (kept in source, not shown)
...
\begin{frame}{A paper accepted at IEEE CoG 2026}
  \begin{columns}[c]
    \begin{column}{0.4\textwidth}\centering
      {\setlength{\fboxsep}{0pt}\setlength{\fboxrule}{0.6pt}%
       \fcolorbox{black!35}{white}{\includegraphics[width=0.92\linewidth,height=0.72\textheight,keepaspectratio]{cog_paper_p1.png}}}
    \end{column}
    \begin{column}{0.6\textwidth}
      ...
      \vspace{0.5em}
      {\small The core of this thesis, condensed into a paper accepted at the
      ...
      \end{tcolorbox}
      \vspace{0.7em}
      {\centering\color{uclBlue}\faMapMarker~~\textbf{Madrid, Spain}\quad
       \textcolor{black!30}{\textbullet}\quad\faCalendar~~\textbf{September 2026}\par}
  ...
  \begin{columns}[c]
    \begin{column}{0.45\textwidth}\centering
      {\setlength{\fboxsep}{0pt}\setlength{\fboxrule}{1pt}%
       \fcolorbox{uclAccent}{white}{\includegraphics[width=\linewidth,height=0.66\textheight,keepaspectratio]{map_bw16x16}}}\\[0.12em]
...
\begin{frame}{Beyond win rate: four game-theoretic metrics}
  ...
  \begin{tabular}{@{}l >{\columncolor{uclBlue!10}}l l@{}}
...
\begin{frame}{Architecture ablation: +21.5 pp}
  \centering
  {\footnotesize\color{black!60}\textbf{\color{uclBlue}7 architectures} $\times$ \textbf{\color{uclBlue}3 seeds}, 100M steps each (\textbf{\color{uclBlue}13.6 GPU-days}); grouped by reasoning axis.\par}
  \vspace{0.45em}
...
\begin{frame}{Feature ablation: signal beats capacity}
  ...
  \vspace{0.3em}
  \includegraphics[width=\linewidth,height=0.76\textheight,keepaspectratio]{feat_ablation_full.png}\\[0.7em]
...
\begin{frame}{Single-map agent: a discount-induced rush collapse}
  \begin{columns}[c]
    \begin{column}{0.6\textwidth}
      \fitfig[0.9\textheight]{rush_collapse_bg.png}
    \end{column}
    \begin{column}{0.4\textwidth}
      \small
      \begin{itemize}\setlength\itemsep{7pt}
        \item \textbf{\color{uclBlue}Setup}: UECD $+$ top-5, \alert{300M steps},
              shaped reward \alert{annealed to sparse} ($1.0\to0$), self-play.
        \item Once shaped $\to0$ with $\gamma<1$, the only way to grow return is
              a \alert{shorter episode}.
        \item 750 $\to$ 300 frames amplifies the terminal signal
              \alert{$\sim$92$\times$}: collapse into a worker-rush exploit.
      \end{itemize}
      \vspace{0.25em}
...
\begin{frame}{Rush fragility on held-out opponents}
  \vspace{0.1em}
  \centering
  {\footnotesize\color{black!55}On held-out opponents the rush policy \alert{\textbf{collapses}} while the balanced one holds:\\
...
\begin{frame}{Why it collapses: a discount-induced shortcut}
  \small
  \textbf{\color{uclBlue}1. Annealing leaves only the discounted terminal reward.}
  \[ J(\pi)\;\xrightarrow{\,w_{\text{shape}}\to0\,}\;
     10\,\mathbb{E}_{\tau\sim\pi}\!\big[\gamma^{T-1}\big]. \]
  With $\gamma<1$, $\gamma^{T-1}$ falls as $T$ grows: maximizing $J$ rewards a
  \alert{shorter episode}, not better play.
  \vspace{1.4em}

  \textbf{\color{uclBlue}2. It reaches the policy through the advantage.}\\
  Two rollouts share $s_t$ but end at $T$ vs $T-\Delta$:
  \[ \hat{A}_t^{(T-\Delta)}-\hat{A}_t^{(T)}
     \;=\;\big(\gamma^{-\Delta}-1\big)\,G_t^{(T)}\;>\;0. \]
  The shorter one always wins, and PPO clips the ratio $\rho_t$,
  \alert{not} $\hat{A}_t$, so the gradient flows.
\end{frame}

\begin{frame}{UECD-Best: two-phase fine-tuning}
  \small
  Resume from the \textbf{150M (pre-collapse) checkpoint} and fine-tune in two
  short phases, keeping the 10\% shaped floor:
  \begin{itemize}\setlength\itemsep{2pt}
    \item \textbf{\color{uclBlue}Phase 1} ($150\text{M}\to240\text{M}$): \textbf{broaden the pool}
          --- swap the 3 weakest bots for TMA $+$ ObiBotKenobi, more self-play.
    \item \textbf{\color{uclBlue}Phase 2} ($240\text{M}\to350\text{M}$): \textbf{harden}
...
% --- UECD-Best: cue slide; the live demo is shown on the project website ---
\begin{frame}{UECD-Best in action on basesWorkers16x16A}
  \vfill
  \centering
  \begin{tcolorbox}[enhanced,colback=uclBlue!5,colframe=uclBlue!30,arc=6pt,boxrule=0.8pt,
      width=0.88\linewidth,left=16pt,right=16pt,top=15pt,bottom=15pt,
      drop fuzzy shadow=black!15]
    \centering
    {\fontsize{38}{38}\selectfont\color{uclBlue}\faLaptopCode}\\[0.6em]
    {\Large\bfseries\color{uclBlue}Watch the final agent play, live}\\[0.55em]
    {\footnotesize\color{black!60}UECD-Best \textcolor{black!45}{vs} RAISocketAI
      \;{\textcolor{uclBlue}{\textbullet}}\; basesWorkers16x16A}\\[0.7em]
    {\small\faGlobe~~\href{https://mathisdelsart.github.io/microrts-drl-uecd-website/}%
      {\color{uclBlue}\bfseries mathisdelsart.github.io/microrts-drl-uecd-website}}
  \end{tcolorbox}
  \vspace{0.5em}

  {\small {\color{uclAccent}\faCrosshairs}~~The agent fields a single military type:
  the \alert{Ranged} unit.}
  \vfill
\end{frame}

\begin{frame}{Tournament: UECD-Best tops the field}
  \centering
  {\footnotesize\color{black!60}19-agent round-robin on basesWorkers16x16A:
  \textbf{\color{uclBlue}96.67\%} win rate, \textbf{\color{uclBlue}first of 19}.\par}
  \vspace{0.3em}
  \fitfig[0.73\textheight]{final_standings_bg.png}
\end{frame}

\begin{frame}{Tournament: the head-to-head matrix}
  \begin{columns}[c]
    \begin{column}{0.56\textwidth}
      \fitfig[0.78\textheight]{h2h_matrix_bg.png}
    \end{column}
    \begin{column}{0.44\textwidth}
      \small
      UECD-Best wins \textbf{\color{uclBlue}100\%} against every agent
      \emph{but two}:
      \vspace{0.7em}
      ...
      \begin{tabular}{l c c}
        \rowcolor{uclBlue}
        \textcolor{white}{\textbf{Opponent}} &
        \textcolor{white}{\textbf{CoG}} &
        \textcolor{white}{\textbf{Sampling}}\\
        WorkerRush  & 50\% & \textbf{99.3\%}\\
        RAISocketAI & 90\% & \alert{\textbf{65.7\%}}\\
        \bottomrule
      \end{tabular}}
      \vspace{0.7em}

      {\footnotesize\color{black!70}\raggedright
      The matrix is the \textbf{CoG} protocol: only \textbf{10 deterministic}
      games per matchup, which produces tie and edge artifacts.\\[3pt]
      We therefore re-evaluate with \textbf{1000 stochastic} games to recover the
      honest win rates (the \alert{Sampling} column).\par}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Beyond win rate: Copeland and Nash}
  \begin{columns}[c]
    \begin{column}{0.5\textwidth}\centering
      ...
      \textbf{how many} opponents you beat, regardless of margin.}\\[0.3em]
      \includegraphics[width=\linewidth,height=0.66\textheight,keepaspectratio]{copeland_scores_bg.png}
    \end{column}
    \begin{column}{0.5\textwidth}\centering
      {\footnotesize \textcolor{uclBlue}{\textbf{Nash}} (in support):
      \textbf{non-exploitable} vs the equilibrium mix; immune to padding the pool.}\\[0.3em]
      \includegraphics[width=\linewidth,height=0.66\textheight,keepaspectratio]{nash_scores_bg.png}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Beyond win rate: $\alpha$-Rank}
  ...
  \textbf{evolutionary stability} --- the strategy hardest to invade and displace.}\\[0.45em]
  \includegraphics[width=\linewidth,height=0.72\textheight,keepaspectratio]{alpha_rank_sweep_bg.png}
\end{frame}

\begin{frame}{Beyond win rate: regret}
  \centering
  {\footnotesize \textcolor{uclBlue}{\textbf{Regret}} (1st avg, 2nd worst-case):
  \textbf{robustness} --- the gap to the pool's best counter for each opponent.}\\[0.45em]
  \includegraphics[width=\linewidth,height=0.72\textheight,keepaspectratio]{robustness_score_bg.png}
\end{frame}

\begin{frame}{Generalization: two distribution-shift probes}
  \centering
  {\small Trained \textbf{only} on \textit{basesWorkers16x16A}, then evaluated under two shifts:}\\[0.25cm]
  \begin{tikzpicture}[
    ...
    arrlbl/.style={font=\footnotesize\bfseries, text=black!65, inner sep=2pt}]
    \node[mapnode, draw=black!45] (train) at (0,0) {\includegraphics[height=2.5cm]{map_bw16x16}};
    \node[mapnode, draw=uclBlue] (scale) at (6,1.35) {\includegraphics[height=2.1cm]{map_bw32x32}};
    \node[mapnode, draw=uecdRed] (layout) at (6,-1.35) {\includegraphics[height=2.1cm]{map_2bb16x16}};
    ...
    \node[anchor=west, align=left, font=\small] at ([xshift=6pt]scale.east)
      {\bfseries\textcolor{uclBlue}{Scale shift}\\[1pt]{\scriptsize\color{black!55}\itshape basesWorkers32x32A}};
    \node[anchor=west, align=left, font=\small] at ([xshift=6pt]layout.east)
      {\bfseries\textcolor{uecdRed}{Layout shift}\\[1pt]{\scriptsize\color{black!55}\itshape TwoBasesBarracks16x16}};
  \end{tikzpicture}\\[0.25cm]
  {\footnotesize\color{black!55} \textbf{100} stochastic-sampling games against \textbf{8} opponents on each map.}
  \vspace{0.35cm}
\end{frame}
...
\begin{frame}{Generalization: scale vs layout}
  \begin{columns}[c]
    \begin{column}{0.7\textwidth}\centering
      \includegraphics[width=\linewidth,height=0.86\textheight,keepaspectratio]{generalization_probes_bg.png}
    \end{column}
    \begin{column}{0.3\textwidth}
      {\small Single-map training\\ \alert{overfits} \textbf{topology}, not \textbf{size}.}\\[1.6em]

      \begin{tcolorbox}[blanker,left=10pt,right=2pt,top=4pt,bottom=4pt,
        borderline west={2.6pt}{0pt}{uclBlue}]
        \footnotesize
        {\textcolor{uclBlue}{\faCheckCircle}~~\textcolor{uclBlue}{\textbf{Scale shift}}~{\scriptsize(32x32)}}\\[2pt]
        {\color{black!65}Win rate \textbf{holds}: tolerated.}
      \end{tcolorbox}
      \vspace{0.45em}

      \begin{tcolorbox}[blanker,left=10pt,right=2pt,top=4pt,bottom=4pt,
        borderline west={2.6pt}{0pt}{uecdRed}]
        \footnotesize
        {\textcolor{uecdRed}{\faTimesCircle}~~\textcolor{uecdRed}{\textbf{Layout shift}}}\\[2pt]
        {\color{black!65}Win rate \textbf{collapses}: brittle.}
      \end{tcolorbox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{The five-map training pool}
  \vspace{-0.2cm}
  \centering
  {\small \textbf{\color{uclBlue}UECD-MultiMap} trains on the five open layouts of size
   $\leq 16{\times}16$, shown here \textbf{to scale}:}\\[0.45cm]
  ...
  \begin{tabular}{@{}c@{\hspace{0.5cm}}c@{\hspace{0.5cm}}c@{\hspace{0.65cm}}c@{\hspace{0.5cm}}c@{}}
    \fcolorbox{black!40}{white}{\includegraphics[height=1.4cm]{map_bw8x8}} &
    \fcolorbox{black!40}{white}{\includegraphics[height=1.4cm]{map_4bw8x8}} &
    \fcolorbox{black!40}{white}{\includegraphics[height=1.4cm]{map_nwtr9x8}} &
    \fcolorbox{black!40}{white}{\includegraphics[height=2.8cm]{map_bw16x16}} &
    \fcolorbox{black!40}{white}{\includegraphics[height=2.8cm]{map_2bb16x16}} \\[0.12em]
    {\scriptsize\bfseries 8x8} & {\scriptsize\bfseries 8x8} & {\scriptsize\bfseries 9x8}
      & {\scriptsize\bfseries 16x16} & {\scriptsize\bfseries 16x16} \\[-0.1em]
    {\tiny\itshape\color{black!55}basesWorkers8x8A} & {\tiny\itshape\color{black!55}FourBasesWorkers8x8}
      & {\tiny\itshape\color{black!55}NoWhereToRun9x8} & {\tiny\itshape\color{black!55}basesWorkers16x16A}
      & {\tiny\itshape\color{black!55}TwoBasesBarracks16x16} \\
  \end{tabular}}
  \\[0.4cm]
  {\footnotesize\color{black!70} Five maps, three sizes, \textbf{\color{uclBlue}different games}.}
\end{frame}
...
\begin{frame}{Extra experiment \#1: multi-map training}
  ...
  \begin{tcolorbox}[enhanced, colback=uclBlue!6, colframe=uclBlue!45, boxrule=0.8pt,
      arc=4pt, left=10pt, right=10pt, top=3pt, bottom=3pt]
    \centering\small
    {\color{uclBlue}\faBullseye}\;{\bfseries\color{uclBlue}GOAL:}\ Can a
    \textbf{Prioritized Level Replay (PLR)} curriculum\\
    on a \textbf{padded environment} train \emph{one} agent across every map size?
  \end{tcolorbox}
  \vspace{0.5mm}
  \begin{columns}[T]
    \begin{column}{0.53\textwidth}\centering
      \fitfig[0.66\textheight]{winrates_per_map_bg.png}
    \end{column}
    \begin{column}{0.47\textwidth}
      \scriptsize
      \begin{tcolorbox}[enhanced, colback=uecdRed!6, colframe=uecdRed!45, boxrule=0pt,
          borderline west={3pt}{0pt}{uecdRed}, arc=2pt, left=8pt, right=6pt, top=3pt, bottom=3pt,
          before skip=0pt, after skip=3pt]
        \raggedright
        {\color{uecdRed}\faExclamationTriangle}\ {\bfseries\color{uecdRed}Problem}\\[1pt]
        On \textbf{8x8} (a \textbf{scale} shift) SingleMap collapses: tiny maps reward
        a \alert{rush} padding cannot carry.
      \end{tcolorbox}
      \begin{tcolorbox}[enhanced, colback=uecdGreen!7, colframe=uecdGreen!50, boxrule=0pt,
          borderline west={3pt}{0pt}{uecdGreen}, arc=2pt, left=8pt, right=6pt, top=3pt, bottom=3pt,
          before skip=0pt, after skip=3pt]
        \raggedright
        {\color{uecdGreen}\faCheckCircle}\ {\bfseries\color{uecdGreen}Fix}\\[1pt]
        Removes the per-map collapse; ranks \textbf{5th overall} (\textbf{64\%} vs
        \textbf{47\%}). Below 50\% only vs the four strongest, on just
        \textbf{200M\,/\,5 maps}.
      \end{tcolorbox}
      \begin{tcolorbox}[enhanced, colback=mStandout, colframe=mStandout, coltext=white,
          arc=2pt, boxrule=0pt, left=8pt, right=6pt, top=3pt, bottom=3pt,
          before skip=0pt, after skip=0pt, drop fuzzy shadow=black!25]
        \raggedright
        {\color{uclAccent}\faCheckCircle}\ {\bfseries The \alert{PLR + padding}
        pipeline works: a multi-map agent is feasible.}
      \end{tcolorbox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[plain,noframenumbering]
  ...
  \begin{tabular}{@{}r@{\hspace{0.5cm}}l@{}}
    {\color{hlTeal}\faEnvelope} & \href{mailto:mathis.delsart@student.uclouvain.be}{\color{white!60}mathis.delsart@student.uclouvain.be}\\
