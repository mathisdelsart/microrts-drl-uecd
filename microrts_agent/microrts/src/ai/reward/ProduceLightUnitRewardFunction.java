...
/**
 * Light unit production reward: +1 per Light unit produced.
 *
 * From RAISocketAI (sgoodfriend).
 */
public class ProduceLightUnitRewardFunction extends RewardFunctionInterface
{
    public void computeReward(int maxplayer, int minplayer, TraceEntry te, GameState afterGs)
    {
        ...
        for (Pair<Unit, UnitAction> p : te.getActions())
        {
            ...
            {
                if (p.m_b.getUnitType().name.equals("Light")) { reward += 1; }
            }
        }
    }
}
