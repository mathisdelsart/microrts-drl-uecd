...
def make_agent_env(
    *,  # force keyword-only
    ...
    if padded:
        from microrts_agent.envs.padded_rl_vec_env import PaddedMicroRTSRLVecEnv

        return PaddedMicroRTSRLVecEnv(max_height=max_height, max_width=max_width, **kwargs)

    from microrts_agent.envs.rl_vec_env import MicroRTSRLVecEnv
...
def make_bot_env(
    *,  # force keyword-only
    ...
    from microrts_agent.envs.bot_vec_env import MicroRTSBotVecEnv
