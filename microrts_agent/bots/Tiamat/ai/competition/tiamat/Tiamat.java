...
public class Tiamat extends AbstractionLayerAI {
    ...
    public Tiamat(UnitTypeTable utt){
        ...
        System.out.println("Tiamat loaded by: " + this.getClass().getClassLoader());

        try {
            Class<?> c1 = Class.forName("ai.asymmetric.SSS.SSSmRTSScriptChoiceRandom");
            System.out.println("SSS class loaded from: " +
                c1.getProtectionDomain().getCodeSource().getLocation());
        } catch (Exception e) {
            System.out.println("SSS class NOT found");
        }

        try {
            Class<?> c2 = Class.forName("ai.configurablescript.BasicExpandedConfigurableScript");
            System.out.println("ConfigurableScript loaded from: " +
                c2.getProtectionDomain().getCodeSource().getLocation());
        } catch (Exception e) {
            System.out.println("ConfigurableScript NOT found");
        }
    }
