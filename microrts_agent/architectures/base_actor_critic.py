...
class GridActorCriticBase(nn.Module):
    ...
    def _finalize(
        ...
        if hl_gauss:
            from microrts_agent.architectures.features.hl_gauss import convert_critic_to_bins
        ...
        if popart:
            from microrts_agent.architectures.features.popart import (
                PopArtNormalizer,
                get_output_layer,
            )
    ...
    def _head_value(self, head, hidden, popart_norm=None, use_tanh=False):
        ...
        if self.hl_gauss:
            from microrts_agent.architectures.features.hl_gauss import hl_gauss_value
