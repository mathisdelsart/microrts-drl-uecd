...
|---|---|
| [`recordings/`](recordings/) | 36 showcase MP4 game clips of `UECD-SingleMap-Best` vs the field. These are the videos shown on the supplementary website. |
...
| [`generalization_probes/`](generalization_probes/) | Generalisation probes of `UECD-SingleMap-Best` (trained on `basesWorkers16x16A`) tested on two unseen maps against 8 opponents (100 games each). CSV summary + markdown table + cleaned per-probe stdout logs. |
| [`BC/`](BC/) | All behaviour-cloning artefacts: [`training/`](BC/training/) (the 6 NPZ teacher chunks, ~65 MB) and [`baseline/`](BC/baseline/) (the 78% BC-only pool WR proof, the horizontal line of the BC+VF→PPO figure). |
| [`agents/`](agents/)           | All 9 trained agents (4 single-map UECD + GridNet single-map baseline + 2 multi-map UECD + 2 BC-family) in *medium* form: inference model + resume checkpoint + config + final eval + training log. ~853 MB total; `UECD-SingleMap-Best` carries an additional `lineage/` subdir reconstructing its full training history from step 0. |
